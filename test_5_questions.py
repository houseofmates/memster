...
# Use the user's personal setup: NIM backend
...
# Signal candidate multiplier: 100 -> 500 candidates for top_k=5
os.environ['SIGNAL_CANDIDATE_MULTIPLIER'] = '100'
os.environ['TOP_K'] = '5'
...
def main():
    print("=" * 60)
    print("NIM Backend + Lightweight Reranker (Two-Stage) - 5 Question Test")
    print("=" * 60)
    
    # Load dataset
    print("[1/3] Loading dataset (first 5 questions)...")
    dataset = load_dataset("oracle")[:5]  # only first 5 for speed
    ...
    for i, question in enumerate(dataset):
        ...
        print(f"  Question {i+1}: {question['question_id']} ({question['question_type']}) -> {'FOUND' if found else 'NOT FOUND'} in {elapsed:.3f}s")
