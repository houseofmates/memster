...
# Signal candidate multiplier: how many candidates each signal fetches
# 500 = matches V6 benchmark (proven 95.20%)
# Lower = faster but lower recall
SIGNAL_CANDIDATE_MULTIPLIER = int(os.environ.get("SIGNAL_CANDIDATE_MULTIPLIER", "500"))
...
# NIM API config
def _get_nim_api_key() -> str:
    """Resolve NVIDIA NIM API key from env or ~/.hermes/.env fallback."""
    key = os.environ.get("NVIDIA_API_KEY", "") or os.environ.get("OPENROUTER_API_KEY", "")
    if key:
        return key
    # Fallback: load from Hermes .env file
...
NIM_API_KEY = _get_nim_api_key()
NIM_MODEL = "nvidia/nv-embed-v1"
NIM_BASE_URL = "https://integrate.api.nvidia.com/v1/embeddings"

# ── NIM embedding helpers (direct HTTP, no external package) ──────

def _nim_embed(text: str, retries: int = 3, input_type: str = "query") -> Optional[List[float]]:
    """Embed via NVIDIA NIM API. Returns 4096-dim vector or None.

    input_type: "query" or "passage" — must be SAME for both storage and retrieval.
    """
    if not NIM_API_KEY:
        return None
    text = text[:6000]
    for attempt in range(retries):
        try:
            ...
            if e.code == 429:  # Rate limit
                ...
                logger.warning(f"NIM embed rate limited (429), retrying in {wait_time:.1f}s...")
                time.sleep(wait_time)
                if attempt == retries - 1:
                    logger.error("NIM embed failed after retries due to rate limit")
                    return None
                continue
            else:
                logger.warning(f"NIM embed failed: {e}")
                if attempt < retries - 1:
...
def _nim_embed_batch(texts: List[str], input_type: str = "query") -> Optional[List[List[float]]]:
...
def embed_text(text: str, input_type: str = "query") -> Optional[List[float]]:
    """Embed text using the configured backend.

    input_type: "query" or "passage" — must be SAME for both storage and retrieval (NIM only).
    """
    if EMBEDDING_BACKEND == "nim":
        result = _nim_embed(text, input_type=input_type)
    ...
    return _nim_embed(text, input_type=input_type)


def embed_batch(texts: List[str], input_type: str = "query") -> Optional[List[List[float]]]:
    """Batch embed using the configured backend."""
    if EMBEDDING_BACKEND == "nim":
        result = _nim_embed_batch(texts, input_type=input_type)
        if result is not None:
            return result
    ...
    if result is not None:
        return result
    return _nim_embed_batch(texts, input_type=input_type)
...
def get_embedding_dim() -> int:
    ...
    if EMBEDDING_BACKEND == "nim":
        # nvidia/nv-embed-v1 produces 4096-dim vectors
        return 4096
...
def _vector_search_sql(query_embedding: List[float], limit: int = 100, threshold: float = 0.1, embedding_column: str = "local_embedding") -> Dict[int, float]:
    """Brute-force cosine similarity search via the specified embedding column.
...
class HybridRetrievalEngine:
    ...
    def __init__(self, db_connection_factory):
        ...
        # Set the embedding column to use for vector search
        if self.embedding_backend == "nim":
            self.embedding_column = "embedding"
        else:
            self.embedding_column = "local_embedding"

        # Initialize backend-specific attributes
        self.local_embedding_model = None  # kept for compatibility, not used
        ...
        if self.embedding_backend == "local":
            ...
            try:
                from memster.local_embeddings import is_local_embedding_available
                if is_local_embedding_available():
                    self.local_embeddings_available = True
                    # Warm up the model to log dimension, but only if not already loaded
                    from memster.local_embeddings import get_shared_local_embedding_model
                    model = get_shared_local_embedding_model(LOCAL_EMBEDDING_MODEL)
                    if model is not None:
                        self.embeddings_available = True
                        logger.info(f"Using local embeddings backend ({LOCAL_EMBEDDING_MODEL}, {model.dimension}-dim)")
                    else:
                        self.local_embeddings_available = False
                else:
                    logger.warning("Local embeddings requested but not available")
            except ImportError:
                logger.warning("Local embeddings requested but sentence_transformers not installed")
            except Exception as e:
                logger.warning(f"Failed to check local embedding availability: {e}")
                
        elif self.embedding_backend == "nim":
            # Check for NIM API key
            self.nim_available = bool(NIM_API_KEY)
            if self.nim_available:
                self.embeddings_available = True
                logger.info("Using NVIDIA NIM embeddings backend (nvidia/nv-embed-v1, 4096-dim)")
            else:
                logger.warning("NIM backend requested but NVIDIA_API_KEY not set")
        else:
            logger.error(f"Unknown embedding backend: {self.embedding_backend}. Falling back to local.")
            # Fallback to local
            ...
            try:
                ...
                if is_local_embedding_available():
                    ...
                    if model is not None:
                        self.embeddings_available = True
                        logger.info(f"Fell back to local embeddings backend ({LOCAL_EMBEDDING_MODEL}, {model.dimension}-dim)")
                    else:
                        self.local_embeddings_available = False
                else:
                    logger.warning("Local embeddings not available even as fallback")
            except ImportError:
                logger.warning("Sentence transformers not installed for fallback")
            except Exception as e:
                logger.warning(f"Failed to load local embedding model as fallback: {e}")

        # Final fallback logic - if primary backend failed, try the other
        if not self.embeddings_available:
            if self.embedding_backend == "local" and not self.local_embeddings_available:
                # Try NIM as fallback
                self.nim_available = bool(NIM_API_KEY)
                if self.nim_available:
                    self.embedding_backend = "nim"
                    self.embeddings_available = True
                    logger.info("Fell back to NVIDIA NIM embeddings backend")
            elif self.embedding_backend == "nim" and not self.nim_available:
        ...
        if not self.embeddings_available:
            logger.error("No embedding backend available - retrieval will fail")
    ...
    def retrieve(
        ...
        signals = {}
        signal_limit = top_k * SIGNAL_CANDIDATE_MULTIPLIER
        ...
        # 3. Fusion — fetch extra candidates for two-stage reranker
        fusion_top_k = top_k * max(self.two_stage_reranker_candidates_multiplier, 1) if self.use_two_stage_reranker else top_k
        if fusion_method == "rrf":
            final = self._reciprocal_rank_fusion(signals, fusion_top_k)
        else:
    ...
    def _semantic_search(self, query: str, limit: int) -> Dict[int, float]:
        ...
        try:
            query_emb = embed_text(query, input_type="query")
